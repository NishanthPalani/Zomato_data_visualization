...
## Below code will display the home tab section of the page
if selected == "Home":

    st.markdown("# Zomato Data Visualization and Exploration")
    st.write(" ")
    st.markdown("### :blue[What is Zomato? :]")
    st.markdown("### Zomato is an Indian multinational restaurant aggregator and food delivery company. It was founded by Deepinder Goyal and Pankaj Chaddah in 2008. Zomato provides information, menus and user-reviews of restaurants as well as food delivery options from partner restaurants in more than 1,000 Indian cities and towns, as of 2022–23. Zomato rivals Swiggy in food delivery and hyperlocal space")
    st.write(" ")
    st.markdown("### :blue[Problem Statement :]")
    st.markdown("### Zomato is a popular restaurant discovery and food delivery service. Data analysis on the platform's data could be used to gain insights into customer preferences and behavior, as well as identify trends in the restaurant industry. In real world these data could be challenging to analyze and interpret. Inorder to make it easy we will use data visualization techniques that can be used effectively to communicate the insights from Zomato data analysis to a wide range of stakeholders, including restaurants, food industry players, and investors")
    st.write(" ")
    st.markdown("### :blue[Overview :]")
    st.markdown("#### This streamlit app aims to give users a friendly environment which can be used to visualize the Zomato data and gain lots of insights on Top restaurants its favourite Cuisine, Number of users using online deliveries and Dine-in. Bar charts, Pie charts and Geo map visualization are used to get better insights.")
    st.write(" ")
    st.markdown("### :blue[Technologies used :]")
    st.markdown("#### - Python, Pandas")
    st.markdown("#### - Streamlit and Plotly Dashboard")
## Below code will be active if "Explore More Data" is selected from side bar where the visual charts can be seen for Analysis
    ...
    with col1:  ## Below option will enable the select box to choose the Specific country to do the analysis
    ...
    with col2: ## These are some additional informations of the selected country to see more details
    ...
    with col1: ## Below code will show the top 5 high rated restaurants in the selected country with a BAR CHART
    ...
    with col2: ## Below code will show the top 8 recommended cuisines in the selected country with a BAR CHART
    ...
    ## Below code will give the heading for the Average Cost of Two ineach city and its line chart
    ...
    ## Below code will give the Bar Chart representation for the Online delivery count in each city
    ...
    ## Below code will give the Bar Chart representation for the Cuisines count in each city
    st.write(" ")
    ...
    ## Below code will show the percentage contribution of online delivery with a PIE chart in the selected country
    with col1:
    ...
    with col2:
        ...
        st.write(fig)
   ## Below code will show the difference of INR currency with the actual country value with a BOX chart in the selected country    
    with col3:
    ...
    ## Below code will give the individual analysis of the selected city
    st.write(" ")
    ...
    ## Below code will give more Insight on each City data 
    with col2:
    ...
    col1,col2,col3 = st.columns([2,2,2],gap="small")
    ## Below code will show the percentage contribution of online delivery, Dine-in services with a PIE chart in the selected City
    ## Along with the rating count, Favorite cuisines in the selected city
    with col1:
    ...
    st.write(fig)
    st.write(" ")
## Below code is only for Data on INDIA report
else:
    ...
    col1, col2 = st.columns([2,2],gap="large")
    ## Below code show the top 5 high rated restaurants in India with a BAR CHART view
    with col1:
        df1 = df[df["Aggregate rating"] == df["Aggregate rating"].max()][["Restaurant Name","Votes","Cuisines"]].sort_values(by = "Votes",ascending=False).head(5)
        fig = px.bar(df1,x="Restaurant Name",y="Votes",color="Restaurant Name",hover_name="Votes",text="Cuisines",color_discrete_sequence=px.colors.sequential.Blackbody,
        labels={"Restaurant Name":"Restaurant Names","Votes":"Total Recommendations"})
        ...
        st.write(fig)
        st.write(" ")
    ## Below code show more insights on India data
    with col2:
        total = int(df["Restaurant Name"].count())
    ...
    col1, col2 = st.columns([2,2],gap="large")
    with col1: ## Below wis used to segregate the heading
    ...
    with col2: ## Below code is to show the top most recommended cuisines in India
        df1 = df_exploded.groupby("Cuisines")[["Aggregate rating","Votes"]].mean().reset_index().sort_values(["Aggregate rating","Votes"],ascending=False).head(8)
        fig = px.bar(df1,x="Aggregate rating",y="Cuisines",color="Cuisines",hover_name="Votes",text="Votes",orientation="h",color_discrete_sequence=px.colors.sequential.YlGn,
        labels={"Cuisines":"Top Cuisines","Votes":"Total Recommendations"})
        fig.update_layout(
    ...
    ## below code will give the heading for the Average Cost of Two in India and its line chart variation
    ...
    with col1: ## Below code is used to select specific City where we can see additional information
    ...
    with col2: ## Below code will give more insight on each city data with its own positive points
        total = int(df1["Restaurant Name"].count())
    ...
    col1, col2, col3 = st.columns([2,2,2],gap="small")
    ## Below code will give the rating count, percentage contribution of table booking, Dine-in Services with a PIE chart in selected City  
    with col1:
        df_count = df1["Rating text"].value_counts().reset_index()
        fig = px.bar(df_count,x="Rating text",y="count",color="Rating text",hover_name="count",color_discrete_sequence=px.colors.sequential.Rainbow,
        labels={"Rating text":"Rating Category","count":"Total Count"})
    ...
    ## Below will show the Map view on the price range and how it scatters between Cities
